...
\subsection{Utilizing sessionInfo() output}

\verb@switchr@ also supports using information from \verb@sessionInfo()@
output at both the seeding and manifest-creation stages.

We can make a \textit{seeding manifest} from the current session info
by calling the \verb@makeSeedMan@ function with no arguments (or with
only the \verb@known_manifest@ argument, if applicable):

<<>>=
makeSeedMan()
@ 

Given the text output of \verb@sessionInfo@ (likely from a different
session than the current one), we can also seed a \verb@switchr@
library with it directly.
...
Note that the \verb@switchTo@ mechanism can take the full
\verb@sessionInfo@ output. No preprocessing or subsetting of the
